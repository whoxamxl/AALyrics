...
Normal Phone timed presentation consumes the active-line fact. The authorized Phone WORD_SYNC Karaoke consumer may additionally consume the shared word/progress/boundary facts downstream without changing timing semantics. The authorized Android Auto Now Playing completion consumes the same shared active-line fact for both LINE_SYNC and WORD_SYNC while intentionally ignoring word/progress/boundary facts. Sync controls, persistence, user calibration, and Android Auto Karaoke remain outside this scope. During Android Auto projection, application policy currently supplies a fixed `-75 ms` audio-path compensation to both Phone and Automotive lyrics consumers. This connection-scoped offset returns to zero when projection ends and does not alter the normalized playback clock, canonical timestamps, or calibration ownership.
