...
The support surface is a compact landing card rather than a generic Settings row. It intentionally avoids a separate full Buy Me a Coffee logo panel so the brand is not repeated. A small locally bundled Buy Me a Coffee cup mark provides a subtle native Compose animation above the copy; no GIPHY iframe, WebView, remote media fetch, or additional tracking/network dependency is introduced. The content order is: animated cup accent, concise native AALyrics explanation, muted optional/external-payment note, increased breathing room, one smaller centered official yellow Buy Me a Coffee button, then a small `Opens Buy Me a Coffee` affordance label. QR assets are intentionally not used on the in-app phone surface.
