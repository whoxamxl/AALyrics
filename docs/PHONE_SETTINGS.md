...
The support surface is a compact landing card rather than a generic Settings row. It uses the user-approved Buy Me a Coffee animated sticker as the single CTA. The repository bundles the original 480×480, 122-frame GIF unchanged; no frame reduction, resizing, or recompression is applied to the asset. The content order is: concise native AALyrics explanation, muted optional/external-payment note, 32dp breathing room, the centered animated Buy Me a Coffee CTA, then a small `Opens Buy Me a Coffee` affordance label. QR assets are intentionally not used on the in-app phone surface.

The original GIF contains substantial transparent canvas around the animated artwork. AALyrics therefore crops only at presentation time with an `ImageView` matrix using the animation's union content bounds (119,187)-(361,288), while leaving the bundled bytes untouched. The GIF is decoded defensively. On Android 9+ AALyrics uses the platform `ImageDecoder` / `AnimatedImageDrawable` path; decode failure or an older Android version falls back to a native static yellow `Buy Me a Coffee` button instead of failing the Settings surface. Tapping either form hands off to the configured Buy Me a Coffee page in a secure browser surface, preferably Android Custom Tabs with ordinary external-browser fallback where necessary. The application/runtime boundary owns launching that external destination; `:ui:phone` emits a support-link callback and must not own Android intents, Custom Tabs, or browser APIs.
