...
- The 60-second token session is adapter-owned and in memory. A rejected HTTP/API token is invalidated, reacquired, and retried once; an unusable initial token response is an operational failure when no cached token exists. Authenticated API 403 responses are operational failures and do not enter the API-401 refresh path. Android `SharedPreferences` is not part of the provider boundary.
