...
In the #75 baseline, these post-Update progress states were presented by the Settings Version/update section after the release-available dialog handed off to the validated download route. The current **#77** checkpoint has moved `PREPARING_DOWNLOAD` and `DOWNLOADING` presentation ownership into the Unified Update Dialog without changing the download boundary or byte accounting. `VerifyingDownload` also remains on that dialog surface during SHA-256 work, with dedicated verification copy deferred to the next checkpoint.
