...
- `APP`:
  ...
  - current #77 checkpoint: the complete post-Update process presentation is dialog-owned: `PREPARING_DOWNLOAD`, `DOWNLOADING`, dedicated `VerifyingDownload`, `DOWNLOADED / Ready to install`, `DOWNLOAD_FAILED / Retry`, `PREPARING_INSTALL`, install-refresh `Newer update available / Download`, `PERMISSION_REQUIRED`, `INSTALLING`, and typed `INSTALL_FAILED / Retry`. Settings is reduced to Idle / manual Checking / manual Up to date / manual Check failed; Ready to install and install Retry use the existing `installUpdate()`, download/retarget actions use the existing `downloadUpdate()`, and Android source-trust / final installer confirmation remain system-owned;
