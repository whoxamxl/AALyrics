...
- `APP`:
  ...
  - current #77 checkpoint: `PREPARING_DOWNLOAD`, `DOWNLOADING`, and dedicated `VerifyingDownload` presentation are dialog-owned; verification uses `Verifying update` / `Checking download integrity…` with indeterminate progress, while downstream Ready to install / install / failure states remain Settings-owned until their corresponding migrations;
