...
- `APP`:
  ...
  - current #77 checkpoint: `PREPARING_DOWNLOAD`, `DOWNLOADING`, dedicated `VerifyingDownload`, and `DOWNLOADED / Ready to install` are dialog-owned; Ready to install keeps an explicit Install action wired to the existing `installUpdate()`, while download failure and install-process/failure states remain Settings-owned until their corresponding migrations;
