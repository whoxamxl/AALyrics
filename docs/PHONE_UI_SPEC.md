...
- `APP`:
  ...
  - current #77 checkpoint: `PREPARING_DOWNLOAD`, `DOWNLOADING`, dedicated `VerifyingDownload`, `DOWNLOADED / Ready to install`, and `DOWNLOAD_FAILED / Retry` are dialog-owned; Ready to install uses the existing `installUpdate()`, Retry uses the existing `downloadUpdate()`, while install-process/failure states remain Settings-owned until their corresponding migrations;
